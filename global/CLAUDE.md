...
Projects with a `.cortex-engine/` directory use Cortex for semantic retrieval and context management. The project's own `CLAUDE.md` contains the full session protocol, CLI commands, agent system details, and initialization/update procedures.

**This global file does not duplicate that content.** It provides only the universal rules above.
